...
/*******************   MAIN    ****************************/
 std::vector<int> clsKeyboardTest::GetUnicodeBuffer (void) {
    int show_keycodes = 1;
    int fd;
...
//               }
 ...
 // ******* CLEAN UP ************************
void clsKeyboardTest::clean_up(int fd ) {
...
void clsKeyboardTest::get_mode(int fd) {
    string m;
...
// ***************** IS A CONSOLE *************************
int clsKeyboardTest::is_a_console(int fd) {
    char arg;
...
// ***************** OPEN A CONSOLE *********************
int clsKeyboardTest::open_a_console(const char *fnam) {
	int fd;
...
int clsKeyboardTest::getfd(const char *fnam) {
	int fd;
   //     printf ("FINDING FD\n");
	if (fnam) {
		fd = clsKeyboardTest::open_a_console(fnam);
		if (fd >= 0) {
	...
	fd = clsKeyboardTest::open_a_console("/proc/self/fd/0");
	if (fd >= 0) {
   //        printf ("FD FOUND   /proc/self/fd/0");     
            return fd;  
        }

	fd = clsKeyboardTest::open_a_console("/dev/tty");
	if (fd >= 0) {
 //           printf ("FD FOUND   /dev/tty");
            return fd;
        }

	fd = clsKeyboardTest::open_a_console("/dev/tty0");
	if (fd >= 0) {
 //         printf ("FD FOUND   /dev/tty0");
            return fd;
        }
	fd = clsKeyboardTest::open_a_console("/dev/vc/0");
	if (fd >= 0)  {
 //         printf ("FD FOUND   /dev/vc/0");
            return fd;
        }
	fd = clsKeyboardTest::open_a_console("/dev/console");
	if (fd >= 0)  {
